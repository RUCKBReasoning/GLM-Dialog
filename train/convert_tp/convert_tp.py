...
SEQUENTIAL_LAYERS = [
    ...
    "block_position_embedding.block_position_embeddings.weight",
    "position_embeddings.weight",
    "cls_mixin.dense.weight",
    "cls_mixin.dense.bias",
]
...
def parse_arguments():
    ...
    parser.add_argument("--input-folder", default=None,
                        type=str, help="Input SAT checkpoint folder")
    parser.add_argument("--output-folder", default=None,
                        type=str, help="Output SAT checkpoint folder")
    parser.add_argument("--target-tp", default=4,
                        type=int, help="Target TP degree")
    return parser.parse_args()


def merge_weights(key, sd_list, tp_index, original_tp, target_tp, cat_dim, is_qkv):
    print(key, is_qkv)
    if original_tp >= target_tp:
        if is_qkv:
            if original_tp > target_tp:
                ...
                for i in range(len(sd_list)):
                    chunks = torch.chunk(sd_list[i][key], 3, dim=cat_dim)
                    part1.append(chunks[0])
                    part2.append(chunks[1])
                    part3.append(chunks[2])
                merged_sd = torch.cat(part1 + part2 + part3, dim=cat_dim)
            else:
        ...
        if is_qkv:
            ...
            chunks = torch.chunk(sd_list[0][key], num_part * 3, dim=cat_dim)
            merged_sd = torch.cat(
                [chunks[offset], chunks[num_part + offset], chunks[2*num_part + offset]], dim=cat_dim)
        else:
            # without clone, torch will save entire tensor
            merged_sd = torch.chunk(sd_list[0][key], num_part, dim=cat_dim)[
                tp_index % num_part].clone()
    return merged_sd
...
def create_checkpoint(sd_list, tp_index, original_tp, target_tp):
    ...
    for key in sd_list[0].keys():
        name = ".".join(
            key.split(".")[3 if key.startswith("transformer.layers") else 1:])
...
def main(args):
    ...
    original_tp = len(glob.glob(os.path.join(
        args.input_folder, iteration, "mp_rank_*_model_states.pt")))
    print(os.path.join(
        args.input_folder, iteration, "mp_rank_*_model_states.pt"))
    ...
    for i in range(0, args.target_tp):
        save_path = os.path.join(
            args.output_folder, iteration, f"mp_rank_{i:02}_model_states.pt")
        ...
        torch.save(create_checkpoint(
            sd_list, i, original_tp, args.target_tp), save_path)
